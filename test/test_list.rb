...
class TestList < Minitest::Test
    def setup
        require_relative '../lib/modules/list'
    end

    def test_list_one_element
        answer = Answer.new('list', 'list', 'maxdeviant', 'theartificiallounge', 'shirt')
        list = answer.list
        assert_equal 'The only *shirt* I know is _t-shirt_.', list
    end

    def test_list_two_elements
        answer = Answer.new('list', 'list', 'maxdeviant', 'theartificiallounge', 'scotch')
        list = answer.list
        assert_equal 'I know 2 kinds of *scotch* : _ardbeg_, _glenmorangie_.', list
    end

    def test_list_no_element
        answer = Answer.new('list', 'list', 'maxdeviant', 'theartificiallounge', 'potato')
        list = answer.list
        assert_equal "I don't know any *potato*.", list
    end

    def test_list_duplicates
        answer = Answer.new('list', 'list', 'maxdeviant', 'theartificiallounge', 'duptest')
        list = answer.list
        assert_equal 'I know 2 kinds of *duptest* : _a_, _b_.', list
    end

    def test_list_all
        answer = Answer.new('list', 'all', 'maxdeviant', 'theartificiallounge', 'list all')
        list = answer.all
        assert_equal 'There are 2 listable topics : _scotch_, _duptest_.', list
    end

    def test_list_help
        answer = Answer.new('list', 'list', 'maxdeviant', 'theartificiallounge', '')
        list = answer.list
