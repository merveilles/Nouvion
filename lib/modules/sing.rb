...
class Answer
    def sing
        ...
        if artist.include? 'by'
            ...
            if ret == "We don't currently have a license for these lyrics. Please try again in a few days!"
                ...
                ret = ret.each_line.reject { |x| x.strip == '' }[0...4].map { |x| ':notes: ' + x }.join
            end
    ...
    def help
        'Make me sing a random artist song by calling `ludivine sing artist` or a specific one with `ludivine sing song by artist`'
    end
end
