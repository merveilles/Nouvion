class Answer
	def paradise
		"A work in-progress implementation of Paradise on Ludivine. The current methods are:\n`count `  `where is `"
	end
  ...
  thoughts = @memory.paradise(query)
		...
		'There are *' + thoughts.length.to_s + "* mentions of _#{query}_ on <http://wiki.xxiivv.com/paradise|Paradise>."
	end
  ...
  vesselLocation = vesselParentArray[0][1]
		...
		"In <http://wiki.xxiivv.com/paradise|Paradise>, *#{vesselName}* is in #{vesselLocation}."
	end
end
